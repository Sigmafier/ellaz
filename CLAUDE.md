...
- **A key never moves or resizes the game.** Arrows, Space, PageDown and End scrolled every game page off its own screen (all 43, live, 2026-09-14), so `src/portal/keyGuard.ts` holds the scrolling keys page-wide while the game is in view - never while typing, never Space on a button, never once the reader has scrolled to the article - and a control that changes with the game's phase keeps one height. `npm run assert:keys` presses them on every game. → [`a-key-a-game-could-use-never-scrolls-the-page.md`](.claude/rules/a-key-a-game-could-use-never-scrolls-the-page.md)
