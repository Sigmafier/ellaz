...
**Since 2026-09-12 it also holds the Toybox ENGINE** (`studio/toybox/`: a pure
...
- **A showcase game's own controls go on an ENTRANCE SCREEN over its arena, never in a row under it.** Title, difficulty, the action button, and whatever that game's chrome owns - drawn while nothing is being steered, with the HUD hidden under it. A row under the arena reserves height on every frame of the run; an entrance reserves it on none, which measured **234px → 359px of arena** at 1536x639 with the phone unchanged. Operator ruling 2026-09-13, picked off four arms rendered over the live game. It is NOT a loading poster - the game has already mounted. Same band as the arcade HUD (`tier: "showcase"`), never a game id.
- **A showcase game's ARENA is landscape on a PC and portrait on a phone, and the shape is a property of the RUN, never a module constant.** Operator ruling 2026-09-13. The two shapes hold the same AREA (420x560 -> 648x364, +0.3%) rather than the same height (996x560 would be +137% of floor, which roughly halves the crowd and is a difficulty change smuggled in as a layout one). It is picked once at mount off the SAME `min-width: 900px` the board's CSS sizes against - two numbers would be two answers - and it must be handed to the simulation, because a media query can change how big a box is drawn and cannot change how many units of floor the game has. **A landscape arena also forces the second panel**: `.ellaz-game-panel` caps at 700px, which is a reading width, and a 16:9 board held inside it is SMALLER than the portrait board it replaces (684x384 against 565x753 at 1920x1080). So the showcase band gets `.ellaz-panel-wide` at 1680px. Measured: **359 -> 853px** of arena at 1536x639 and **565 -> 1638px** at 1920x1080, phone byte-identical at 359. **Since 2026-09-14 that arena is the VIEW, not the floor** (operator ruling, *"map to go to the sides"*): `run.world` is three views wide and tall, the camera follows the robot and stops at striped walls, shapes enter just outside the view and are walked back in once a whole view behind, and a run picks one of three weapons, carries up to four (blades and a drone among them) on their own clocks, auto-dashes out of one hit per 8 s and freezes everything for 2 s on a gem-charged button. Rules in `survivors/world.ts`, `arsenal.ts`, `powers.ts`, `cards.ts`; `scripts/repro/repro-survivors-expansion.mjs` checks it in a browser.
- **Steering is one component, and it binds the KIDS BAND** — `DirectionPad` from `@ui/DirectionPad`, imported by its own path (it lives in the `page` chunk, so a barrel re-export would make the shell import from it). In an `ageBand: "kids"` game, never ship the joystick alone: the four arrows are what make it tap-completable, and a five-year-old cannot hold a sustained drag. **A showcase-tier game may instead put the steering ON the arena** — operator ruling, 2026-09-13, on `survivors`: a stick born where the thumb lands, with a fixed-corner alternative on the game's own chrome, the pad removed, and keyboard still live on desktop. The law was NARROWED in the same change that first broke it, not afterwards. Measured when it was narrowed: `maze` is the only `ageBand: "kids"` game importing the pad, so the band it still binds is a real one rather than an empty exemption. **And a Controls setting, 2026-09-14** (operator: *"the joystick should be a setting in every game with movement"*): snake and maze offer Arrows / Joystick / On the board through `ControlModePicker` + `useControlMode`, remembered per game under the forever key `controlMode`. **Arrows is the default and one tap away**, which is what keeps the kids band tap-completable; the big stick (`DirectionPad variant="stick"`) and the on-board stick (`BoardStick`) are alternatives a player chooses, never the shipped state.
- **A key never moves or resizes the game.** Arrows, Space, PageDown and End scrolled every game page off its own screen (all 43, live, 2026-09-14), so `src/portal/keyGuard.ts` holds the scrolling keys page-wide while the game is in view - never while typing, never Space on a button, never once the reader has scrolled to the article - and a control that changes with the game's phase keeps one height. `npm run assert:keys` presses them on every game. A difficulty change must not resize it either - `npm run assert:difficulty` taps every level. → [`a-key-a-game-could-use-never-scrolls-the-page.md`](.claude/rules/a-key-a-game-could-use-never-scrolls-the-page.md)
...
- **Every game has a PC version, and it is CENTRED** (operator rulings, 2026-09-14). A board declares numbers through `boardVars()` + `BOARD_CLASS` (`@ui/boardSize`) and `.ellaz-board` owns both arms: the phone keeps `min(vw, vh, cap)`, a PC grows to the height the window leaves. On a PC the panel has no width cap (only its row keeps 700px), a game's own footer sits in a column BESIDE the board with an empty column of the same width on the other side, and a free-play arena fills that width through `pcFillArena()` with props scaled, never added. `scripts/repro/repro-board-fills-the-window.mjs` refuses a board under 90% fill, a stale `chrome` number, a moved phone frame, or a board more than 2px off the screen's centre line. **And nothing beside the board is ever out of sight**: a picker goes in `GameChrome`'s `side` (the left column on a PC), a sideways `.ellaz-strip` wraps, a too-tall column is scaled to fit, and `npm run assert:on-screen` refuses any control past the window or behind a scrollbar at five PC sizes - the board gate was green over five games with cut controls. → [`a-gate-that-measures-the-board-cannot-see-the-controls.md`](.claude/rules/a-gate-that-measures-the-board-cannot-see-the-controls.md)
- **On a phone, a game page has ONE 52px bar** - home, name, pause, restart, sound, and a "more" button holding language, share, full screen, tell us and the coins - and the game's panel fills the box to the bottom. Operator ruling 2026-09-14, picked off real renders. The geometry is CSS (`--hh:52px; --uh:0` under 720px on `body[data-page="game"]`, so nothing shifts), the controls are MOVED by `src/portal/phoneBar.ts` (never copied - each is wired once by `querySelector`), and a showcase arena takes the phone's shape at the same floor area (`survivors/phoneArena.ts`). The room, the boards and a PC keep both rows. → [`game-controls-and-platform-chrome-never-share-a-bar.md`](.claude/rules/game-controls-and-platform-chrome-never-share-a-bar.md)
...
- **A font subset's `unicode-range` is not the script it is named after.** Google's Heebo `hebrew` face also claims `U+200C-2010` - the zero-width joiner inside every emoji sequence - so English pages fetched 12,000 B of Hebrew to draw an invisible character. `scripts/fonts/sync-fonts.mjs --check` guards it, and "declare the one you want last so it wins" was measured to be false. → [`a-font-behind-an-import-is-invisible-to-every-preload.md`](.claude/rules/a-font-behind-an-import-is-invisible-to-every-preload.md)
- **The styles a document needs ship INSIDE it.** The shell stylesheet is an inline `<style>` in every emitted page, not a `<link>`: a separately-cached file can be absent at paint time and the page then renders naked - which is what the operator saw on 2026-09-08, proven by the one styled element on that screen being the consent bar, whose CSS was already inline. The first visit got 75 B gz SMALLER and a render-blocking request disappeared; `dist/` grew 21 MB to 23 MB. → [`a-stylesheet-in-another-file-can-be-missing-at-paint-time.md`](.claude/rules/a-stylesheet-in-another-file-can-be-missing-at-paint-time.md)
