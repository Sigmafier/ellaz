...
| Ministry portal - pop.education.gov.il | `hebrew-directories.md` | draft | — | — | **The last unsent door of the four, and it is the only one with no form at all** - `mailto:pop@education.gov.il`, verified as the portal's own address on its digital-tools page 2026-08-29. `.gov.il`, 33 dofollow outbound over 19 distinct hosts including Nearpod, BrainPOP and Padlet. It is DOWNSTREAM of `digitalpedagogy.co` by measurement, not by guess: this portal already cites her, and that letter went the same day, so the ordering the lane was planned around actually held. The letter body is ready and byte-stable (`sha256` head `bd9d4274`); nothing was drafted because the Gmail session had expired. | YOU | Send it from your own mailbox to `pop@education.gov.il`. Keep the sentence saying Arabic is INTERFACE-ONLY and that there are no Arabic articles - overstating language coverage is the one error this reader will check. Re-auth Gmail and I will draft it, or copy the body off the board. |
