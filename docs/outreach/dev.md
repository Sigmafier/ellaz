...
The number I care about most is the first visit: **55,097 bytes gzipped**, of a ceiling of
56,000 that fails the build. 903 bytes of room left. That figure is measured on the built
...
| 55,097 B gz first visit, 903 spare | the `Build for the root domain` step of the deploy for `1b8f2b9`, 2026-08-22. **Read off CI, not off a workstation**: the same commit measures ~50 B differently on Node 24, and CI is the toolchain that builds the live site |
