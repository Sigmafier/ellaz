...
| https://dev.to/ytrofr/a-button-that-covers-another-button-passes-every-test-you-have-1j9b | dev.to - our own technical article, *A button that covers another button passes every test you have* | expected | — | 2026-12-02 | **Published 2026-09-03 10:48 UTC** (`places-posts.md`), 864 words, 5 code blocks, canonical self, no `canonical_url` pointing elsewhere. **Its positive control is the state it is in RIGHT NOW: `pointsAtUs` reads `87532 B, NAMED, NOT LINKED`** — the closing paragraph writes `ellaz.fun` and `github.com/Sigmafier/ellaz` as bare text, so our domain is on the page and in no anchor. That is the reading this row exists to change, and it is worth something precisely because the matcher can currently say NO: the same `linkShape` returns `linked:true` on `<a href="https://ellaz.fun/">` and `linked:false` on prose naming us, checked the same minute. **A promotion here takes an ANCHOR, never the name** — which is the whole reason `expected` promotes on `linked` and not on `named`, and this row is the first one where the difference is not hypothetical. **DISCOVERY under RCH13 whichever way the `rel` reads**, because we placed it; the `rel` is nonetheless UNMEASURED and unmeasurable until an anchor exists, since this page carries no user-supplied external anchor of any kind today. One other author's dev.to body link to their own site measured `noopener noreferrer` with no nofollow — a LEAD about their artifact, not a prediction about ours (Newgrounds surveyed 9 dofollow author anchors and gave us two `nofollow`s). |
<!-- /backlinks:rows -->
