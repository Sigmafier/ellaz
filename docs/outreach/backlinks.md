...
| https://osgameclones.com/ | osgameclones - the open-source game clones list, our 8 entries | expected | — | 2026-12-01 | **PR #5052 opened 2026-09-03** (`osgameclones.md`), 8 entries in `games/e.yaml`, OPEN and MERGEABLE. **ONE row, not eight, and that is the correction**: the plan promised a row per game, but this site is a single 6.8 MB page with no per-game URL - eight rows would be eight fetches of one document, which reads as eight instruments and is one. Measured 2026-09-02: **9,871 anchors, ZERO nofollow**, last merge 2026-09-01. Measured again 2026-09-03 as the positive control for this row: the page carries **zero occurrences of `ellaz`**, so the checker can currently say NO, which is what makes a later YES worth anything. A dofollow link here would be the first EARNED one this site has (RCH13); until the PR merges this row must read `expected`, never `live`. |
| https://dev.to/ytrofr/a-button-that-covers-another-button-passes-every-test-you-have-1j9b | dev.to - our own technical article, *A button that covers another button passes every test you have* | live | 2026-09-03 | 2026-12-02 | **Published 2026-09-03 10:48 UTC, and LINKED at 12:0x after a second edit** (`places-posts.md`). It went out with the site named twice and linked zero times - the draft wrote `ellaz.fun` and `github.com/Sigmafier/ellaz` with no scheme, and dev.to does not autolink a bare domain - so for its first hour this row's reading was `87532 B, NAMED, NOT LINKED`. **That failing state was this row's positive control and it is the reason the promotion means anything**: the same matcher, on the same page, said NO before the edit and `87607 B, linked` after it, so `expected` promoting on `linked` rather than on `named` decided this row rather than merely describing it. **`rel` measured on the LIVE page, not the editor's preview: `rel="noopener noreferrer" target="_blank" on BOTH anchors, ZERO nofollow`** - body anchors went 4 (all its own table of contents) to 6. The preview had rendered the same `rel` beforehand, and that was recorded as a LEAD rather than an answer, because Newgrounds surveyed 9 dofollow author anchors and then gave us two `nofollow`s (`.claude/rules/a-survey-of-their-artifacts-is-not-a-prediction-about-yours.md`); here the live page agreed with the preview, which is a fact about this page and still not a prediction about the next one. **This is the FIRST dofollow anchor to ellaz.fun that exists anywhere** - the other four `live` rows are three itch listings and our own repository, and all four are `nofollow`. **It remains DISCOVERY under RCH13, never EARNED, because we placed it ourselves**, and it must not be booked against the 2026-11-27 or 2026-12-01 verdicts. What it does buy is real: a dofollow from a high-authority domain, and the first click-through path this site has had. |
| https://libregamewiki.org/Ellaz | libregamewiki - the Ellaz article, a collection of 42 MIT-licensed browser games | expected | — | 2026-12-02 | **SAVED 2026-09-03 and HELD IN MODERATION** (`libregamewiki.md`). **This row will read `unchecked` and that is CORRECT, not a broken probe.** The wiki runs the Moderation extension: a pending edit is served to nobody, so `https://libregamewiki.org/Ellaz` returns **HTTP 404** to an anonymous fetch, `classify` turns any non-`ok` response into `unchecked`, and it stays there until a moderator approves. **The save itself is the best-evidenced one in this whole ledger.** `Special:Contributions/Ytrofr` is empty and `editcount` is 0, which is how the extension presents a queued edit and is ALSO how a failed save presents - so that reading was treated as a non-verdict and a stronger one was taken: a same-origin `fetch` of `?title=Ellaz&action=edit` returns a textarea holding **2,702 bytes hashing to `3c337d2b7481cf6e`, byte-identical to `docs/outreach/libregamewiki.md`'s copy**, with a **negative control** - the same code path on a page nobody has ever edited returns 0 bytes - so the method can say NO. The server is holding our text. **The positive control for the LINK half is today's 404 plus zero occurrences of `ellaz` on the page**, so the checker is on record saying NO before it is ever trusted to say yes; it promotes only on `linked`. Measured across the wiki 2026-09-03: **695 external anchors, every one dofollow, across 497 distinct hosts**. DISCOVERY under RCH13, never EARNED - we placed it. |
<!-- /backlinks:rows -->
