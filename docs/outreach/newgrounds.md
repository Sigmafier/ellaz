...
Built as a plain web page, about 228 KB, works offline. One game out of a free site of 42.
...
| about 228 KB for 2048 | `dist-standalone/2048` summed on the built artifact, **re-measured 2026-08-29 at HEAD 167f7d2: 233,133 B = 227.7 KB**. It said 204 KB from 2026-08-22, then 207.9 KB the same afternoon, and 227.7 KB once the build stopped stubbing out 2048's own game module - the 204 and 208 figures were both measurements of a bundle that DID NOT RUN. **NOTHING GATES THIS NUMBER** - `assert:outreach` checks the payload ceiling and the CI payload record, not standalone bundle sizes - so it drifts silently with every build and must be re-measured on the day a listing goes up |
