...
//
// A COPY of `studio/adapters/canvas/draw-frame.ts` - see `./manifest.ts` for
// why this is copied rather than imported, and `./player.ts` for why neither is
// re-exported from the `@shared` barrel. The ONLY change is the import path.
//
// `imageSmoothingEnabled = false` below is the whole reason the art survives
// being scaled: every authored pixel is stored as a flat block, so a smoothed
// upscale would blur exactly the edges the style is made of.
