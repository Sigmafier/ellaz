...
/**
 * WHY EVERY DOCUMENT PRELOADS ITS BODY FACE, AND ONLY THE SUBSET IT DRAWS.
 *
 * What stood in the head until 2026-09-07 was a preconnect to
 * fonts.gstatic.com and a stylesheet link to fonts.googleapis.com, emitted on
 * content pages ONLY - so the app shell at the site root received neither, and
 * Lighthouse reported "no origins were preconnected" while this file plainly
 * emitted one. Worse, `global.css` also `@import`ed the same URL for everyone,
 * so a game page loaded the same two families twice, over two origins, by two
 * different paths.
 *
 * Both are gone. The fonts are ours now (`src/ui/fonts.css`, generated by
 * `scripts/fonts/sync-fonts.mjs`) and one `<link rel=preload>` starts the body
 * face downloading WITH the document, rather than after the stylesheet that
 * declares it has itself arrived and parsed.
 *
 * Fredoka is deliberately NOT preloaded: it is a display face for headings and
 * game labels, and its Latin subset alone is 29,704 B - larger than the body
 * face - because it carries a whole 300-700 variable axis. It is declared in
 * CSS and arrives moments later under `font-display: swap`.
 *
 * This comment is a TS comment and not an HTML one on purpose. An HTML comment
 * in the template below is SERVED, on all 246 emitted documents; the reason a
 * tag exists is for whoever reads this file, not for whoever downloads the
 * page. (Measured: as an HTML comment it added ~1.1 KB to every page.)
