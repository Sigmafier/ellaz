  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={
          {
            // PC: the board's width comes from `.ellaz-board` and a cell is one
            // track of it, in `cqw` against this element's content box (the 6px
            // padding is outside it, the gaps are subtracted). The grid tracks
            // are `1fr` so the board's own template never reads its own `cqw`.
            ["--cell" as string]: pc ? `calc((100cqw - ${(size - 1) * CELL_GAP}px) / ${size})` : cell,
            ...(pc
              ? {
                  // chrome 161 is an ESTIMATE: the 111 every GameChrome game
                  // pays plus the 36px hint footer and its 14px padding.
