...
export function Scene({
  ...
  // Re-render when the lazy half of the art lands. `useSyncExternalStore` and
  // not a `useState` in an effect because every shop card renders one of these
  // and they all subscribe to the same one event, which is the shape React
  // gives for exactly that.
  useSyncExternalStore(subscribeRoomArt, roomArtRevision, roomArtRevision);

  // Fetch it, from the first room that mounts. On idle rather than
  ...
  // Idempotent, so eighty cards asking is one request. The World screen does
  // not rely on this - it imports the chunk statically, so its shop is never
  // drawn with the fallbacks (see `World.tsx`).
  useEffect(() => {
    const idle = (
      globalThis as { requestIdleCallback?: (cb: () => void) => void }
    ).requestIdleCallback;
    if (idle) {
      idle(loadRoomArtRest);
      return;
    }
    const timer = setTimeout(loadRoomArtRest, 600);
    return () => clearTimeout(timer);
  }, []);

  const draw = (category: ItemCategory) =>
    roomPiece(category, artFor(category, equipped[category]))();
