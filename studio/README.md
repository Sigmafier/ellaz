...
npm run assert:all     # the nine gates
...
| `games/<name>/` | a game on the engine: `data/`, `assets/`, `tapes/` (with the goldens beside), `page/`, and optionally `tournament/` history. Four today, on three sim kinds: `games/fight/` ([README](games/fight/README.md) - Toybox Brawl, three levels and a boss at `?campaign=brawl`), `games/crypt/` ([README](games/crypt/README.md), 2026-09-12 - the first game built ON the engine; one data-gated rule and three painter kinds), `games/ember/` ([README](games/ember/README.md), 2026-09-13 - a turn battle, the second kind) and `games/hollow/` ([README](games/hollow/README.md), 2026-09-13 - an isometric room, the third kind). A `data/campaign/<id>.json` makes a game whole: a level pick, TRY AGAIN, VICTORY ([`toybox/README.md`](toybox/README.md) § A campaign). Gate: `npm run assert:fight` walks every game. Verdict that picked Phaser: `docs/engine-tournament/fight-2026-09/` |
