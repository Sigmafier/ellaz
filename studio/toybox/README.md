...
| `harness/` | `run-tape.mjs` (the admission gate), `assert-equal-work.mjs`, `bytes.mjs`, `sweep.mjs`, `write-golden.mjs`, `copy-sprites.mjs` - each takes `--game <name>` (fight by default); `record-tape.ts` (a fight tape recorded from the scripted hero in `sim/scripted-hero.ts`, `--after` chaining earlier levels' carry, `--check` re-recording it) and `drive-campaign.mjs` (a built campaign driven title to VICTORY with one deliberate loss; fight kind only today) | `../scripts/lib/` |
...
node toybox/harness/drive-campaign.mjs --game fight --campaign brawl    # title -> a loss -> 4 levels -> VICTORY
