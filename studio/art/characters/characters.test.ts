...
describe("the cast", () => {
  it("twelve characters - six heroes, four small enemies, two bosses - across three bands, unique ids", () => {
    expect(CHARACTERS).toHaveLength(12);
    expect(CHARACTERS.filter((c) => c.role === "hero")).toHaveLength(6);
    expect(CHARACTERS.filter((c) => c.role === "enemy")).toHaveLength(4);
    expect(CHARACTERS.filter((c) => c.role === "boss")).toHaveLength(2);
    expect(new Set(CHARACTERS.map((c) => c.band)).size).toBe(3);
