...
/**
 * vendor-<engine> is the library, fight-core* is the shared sim and harness,
 * everything else is what this cell's own author wrote. Keyed on the chunk
 * NAME the build gives it, so a cell cannot hide a library inside its entry
...
export function bucketOf(basename) {
  ...
  if (/^fight-core/.test(basename) || /^modulepreload-polyfill/.test(basename)) return "shared";
  return "authored";
}
...
function main(argv) {
  ...
  console.log(`bytes: ${cells.length} cell(s) under ${dist}/cells - gzip level 9, declared JS only`);
  if (cells.length === 0) { console.log("bytes: NO cells found - nothing to weigh"); return 2; }
  const rows = cells.map((c) => measureCell(dist, c)).sort((a, b) => a.total - b.total);
  table(rows);
  let bad = 0;
  for (const r of rows) {
    ...
    appendRow({ kind: "bytes", at: new Date().toISOString(), cell: r.cell, engine: r.engine, shared: r.shared, authored: r.authored, total: r.total });
  }
