...
| 2026-09-05 the report inbox listing | an issue title built as `message.slice(0, 60)` | **two of one report's three asks.** A player wrote that power-ups should animate their effect, that two of them swapped together should combine, and that the five-line gem should work on any colour. The listing showed *"Super powers should do an effect when they go to animate the"* and stopped. Triage from that title fixes a third of the report and closes the issue - and the shortfall is invisible, because the title is a faithful prefix of a sentence nobody re-reads |
| 2026-09-05 studio export's dirty stamp | `git status --porcelain -- studio` run with cwd `studio/` | **every dirty tree.** The pathspec named `studio/studio`, a directory that does not exist, so the status was empty and the stamp read clean - on a tree with a modified runner and a whole new `export/`. The same `-dirty` suffix the standalone gate depends on, and it could never be produced. Pathspec `.`, and it was watched printing `-dirty` before the fix was committed |
| 2026-09-05 the rainbow blast's colour cell | `arm.toContain("gemOf(grid[t]")` | **which PROPERTY used it.** A planted defect painting every flying head the SOURCE gem's colour instead of the target's SURVIVED, because the glow underneath the background still named the target and one loose substring match over the whole arm was satisfied by it. Two lines apart in the same element, and the assertion could not tell them apart. It names the declaration it wants now - `background: gemOf(grid[t] ?? 1).fill,` - and the same mutation reds |
| 2026-09-05 studio runner bundle freshness | `mtime(bundle) >= newest(art/, runner/)` | the other two directories the bundle imports (`export/`, `adapters/`). A function added to the runner reported `window.studio.renderTechniqueTile is not a function` - read cold, that says the source lacks it, and the source plainly had it. A freshness check lists its inputs by hand, so it is a hand-kept mirror of an import graph ([`a-path-filter-is-a-hand-kept-mirror-of-an-import-graph.md`](a-path-filter-is-a-hand-kept-mirror-of-an-import-graph.md)) |
| 2026-09-08 "did we stop downloading it?" | a COUNT of resource-timing entries, and `encodedBodySize` | **the difference between a download and a replay.** An entry exists for a cache hit too, and `encodedBodySize` reports the file's full size either way - so "3 woff2 -> 2" was being read off the one field that cannot express the question. A long-lived tab that had visited `/he/` showed FOUR font entries on the English home, on a page with zero rendered Hebrew: it read as a regression in the fix that had just shipped. Printing `transferSize` settled it in one look - every one was **0 bytes, `deliveryType: "cache"`**. The arms that can answer are a FRESH tab and a caches-cleared reload, where an absent entry cannot be a replay; the claim that holds is about what a page NEEDS, not what a warm browser re-enters into its font set |
| 2026-09-07 the Hebrew-subset hunt, three instruments in a row | (a) a page loaded from the production build, (b) a scan for `[\u0590-\u05FF]`, (c) `getComputedStyle` on a link | (a) **the service worker.** `npm run preview` serves the built app, which REGISTERS one - `CLAUDE.md`'s own QA gotcha says so - so the first three readings were of a precached document, and the tell was a `workbox-precache` cache key I had not looked for. (b) **the other three-quarters of the font's range.** Heebo's `hebrew` face declares `U+0307-0308, U+0590-05FF, U+200C-2010, U+20AA, U+25CC, U+FB1D-FB4F`; my regex covered ONE of those six spans, so a page I certified as having "zero Hebrew characters" was full of the codepoint that actually pulled the file - `U+200D`, the joiner inside 🧑‍🤝‍🧑. (c) computed style said the language picker's link was unstyled while the footer's identical link was styled, which sent me hunting a cascade bug that did not exist. **The one instrument that was right was the one that measured the DOWNLOAD**: `initiatorType: "css"` at 1244 ms, long after the autonym had been on screen |
| 2026-09-07 the accessible-name probe, twice | first `innerText` whole, then `innerText` with emoji and punctuation stripped | first **the emoji**: "🎲 All" beside `aria-label="All"` is not a finding, and counting it reported 12. Then **the aria-hidden art**: axe reads the visible text with screenReader semantics, so the SVG digits inside a game tile (`5 9 7 2 4` on Sudoku) are not part of the comparison, and counting them reported 9. Four survived a walker that skips `aria-hidden`. And in between, a rewrite of the punctuation class pasted `U+2000` as a plain space, turning `[\u0020-\u206F]` into a class that strips every letter - every element's visible text became `""`, nothing could mismatch, and **the planted control read PASS**. Non-ASCII in a pattern goes in as `\u` escapes. **It then happened a THIRD time, on the live-site verification run, after this row was already written** - a pasted literal is mangled every time and reading the rule does not stop it. Build the class with `new RegExp("[\\u2000-\\u206F...]")` so nothing non-ASCII is ever in the transmitted source |
| 2026-09-13 match3 before/after sweep | a real-browser sweep of all 43 games against `localhost:5180` | **the fix.** 5180 was `vite preview` serving a `dist/` built before the edit, so the "after" sweep printed 43 of 43 unchanged and match3 still on two lines - which reads as a fix that does nothing, not as an instrument pointed at the past. `/proc/<pid>/cmdline` said `vite preview`; one `npm run build` later the same sweep showed exactly one game change. Ask of any served-page reading: *which build is this server serving?* |
| 2026-09-05 a `str.replace` patch, twice | `s = s.replace(anchor, new)` with no check that `anchor` occurred | the whole edit. The anchor had drifted one edit earlier, `replace` returned the input unchanged, the file was written back identical, and the script printed its own success line. Found only when the artifact built from it lacked the function. `assert anchor in s` before every replace - the same "assert the mutation landed" this rule already asks of a harness, one layer down |
