...
//
// The globals check runs against COMMENT-STRIPPED source. It used to run
// against raw text, which meant the very first line of protocol.ts —
// "The WebSocket protocol — every message is one JSON object" — tripped the
// /\bWebSocket\b/ ban. Nothing was impure; the gate could not tell a use of a
// global from a mention of one. A gate that fires on its own documentation
// gets suppressed, and a suppressed gate protects nothing.
 ...
 * would read as the start of a comment. `stripComments handles the constructs
 * this codebase actually contains` below pins the real cases, including the
 * one regex literal in engine/cards.ts.
 */
