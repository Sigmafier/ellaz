...
//
//   2. "the page loads" — true of a client whose VITE_HOLDEM_SERVER was never
...
if (code) {
  const welcomed = await new Promise((resolve) => {
    ...
    try {
      ...
      ws.onopen = () =>
        ws.send(JSON.stringify({ t: "hello", v: 2, token: `gate-${Date.now()}-abcdefgh` }));
      ws.onmessage = (ev) => {
  ...
  else if (welcomed.v !== 2) bad(`server speaks protocol v${welcomed.v}, this build expects v2`);
...
if (bundle) {
  ...
  if (text.includes(origin)) {
    ...
    bad(`the bundle still points at localhost — VITE_HOLDEM_SERVER was not set for this build`);
  } else {
